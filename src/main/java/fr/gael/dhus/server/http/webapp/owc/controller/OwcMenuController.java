			...
			try {
				...
				for (MenuSectionsData section : body.getSections()) {
					...
					if (section.getRemoteUrl() != null
						...
						fileWriter.println("\"remoteUrl\":\"\"");
					if (section.getMaxInstances() != null
							&& !section.getMaxInstances().isEmpty())
						fileWriter.println(",\"maxInstances\":\""
								+ section.getMaxInstances() + "\"");					
					fileWriter.println("},");
				...
				logger.error(" Cannot write menu configuration file ");
				e.printStackTrace();
				return new ResponseEntity<>("{\"code\":\"unauthorized\"}",
						HttpStatus.UNAUTHORIZED);
			}			
		} else {
			logger.error(" Cannot get menu configuration file ");
			return new ResponseEntity<>("{\"code\":\"unauthorized\"}",
					HttpStatus.UNAUTHORIZED);
		}
